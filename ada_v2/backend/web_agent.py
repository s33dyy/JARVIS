...
# 1. Load API Key (deferred — settings.json env vars are applied after imports)
def _get_api_key():
...
SCREEN_WIDTH = 1440
SCREEN_HEIGHT = 900
# Use the specific Computer Use preview model, with fallback
...
class WebAgent:
    def __init__(self):
        ...
        if not api_key:
            raise ValueError("GEMINI_API_KEY is not set. Configure it in settings.")
        ...
        async with async_playwright() as p:
            ...
            for turn in range(MAX_TURNS):
                ...
                try:
                    ...
                    # If quota exceeded on computer-use model, try fallback
                    if "quota" in error_str or "exhausted" in error_str or "429" in error_str:
                        print(f"[WARN] Primary model quota exceeded, trying fallback: {FALLBACK_MODEL_ID}")
                        try:
                            response = await self.client.aio.models.generate_content(
                            ...
                            print(f"[CRITICAL] Fallback model also failed: {e2}")
                            if update_callback: await update_callback(None, f"Error: {e2}")
                            break
                    else:
